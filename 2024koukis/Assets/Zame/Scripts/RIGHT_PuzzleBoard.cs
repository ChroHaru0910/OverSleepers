...
public class RIGHT_PuzzleBoard : MonoBehaviour
{
    // 親オブジェクト
    [SerializeField] GameObject ParentObj;

    // ポジションをリストに保存
    List<RIGHTWolrdVec2> objList = new List<RIGHTWolrdVec2>();
    ...
    void Initialize()
    {
        ...
        vec2Rows = new RIGHTWolrdVec2[rows.Length];

        for (int i = 0; i < rows.Length; i++)
        {
            // 各 `WolrdVec2` インスタンスを初期化
            vec2Rows[i] = new RIGHTWolrdVec2();
    ...
    // リストのデータを渡す
    // ゲームマネージャー.csに
    public List<RIGHTWolrdVec2> ReturnList()
    {
        return objList;
    }

}
public class RIGHTWolrdVec2
{
